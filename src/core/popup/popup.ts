document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadStats(): Promise<void> {
    try {
      const response = await chrome.runtime.sendMessage({ type: 'GET_API_CALLS' });
      if (response) {
        apiCallsCountElement.textContent = response.length.toString();
      }

      const sessionResponse = await chrome.storage.local.get('spectrion_current_session');
      if (sessionResponse.spectrion_current_session) {
        const session = sessionResponse.spectrion_current_session as any;
